...
var UIController = (function() {
	...
	var formatNumber = function(num, type) {
		...
		num = Math.abs(num);  // yes, we are overwriting the num argument
		num = num.toFixed(2);  //2 decimal places, always

		numSplit = num.split('.');
		int = numSplit[0];
		if (int.length > 3) {
			int = int.substr(0, int.length - 3) + ',' + int.substr(int.length - 3, int.length);  //converts 2310 to 2,310
		}
	...
	return {
		...
		displayPercentages: function(percentages) {
			...
			// this method was originally just in this method, but we wanted to use it for the changedType method. However its not in its scope, so we had to move it!
			// var nodeListForEach = function(list, callback) {
			// 	for (var i = 0; i < list.length; i++) {
			// 		callback(list[i], i);
			// 	}
		...
		displayMonth: function() {
			...
			// Cool code from a guys suggestion so we dont have to create an array!
